...
const SCRIPTS: Step[][] = [
  [
    { delay: 0, t: "session.start", from: "demo@example.com", subject: "tower defense with a final boss", body: "tower defense with a final boss", ts: Date.now() },
    ...
    { delay: 200, t: "agent.thinking", agent: "indexer", tokens: "Roblox waypoint follow and RemoteEvent patterns" },
    ...
    { delay: 200, t: "agent.thinking", agent: "builder", tokens: "spawning enemies and path-follow logic" },
    ...
    { delay: 800, t: "agent.thinking", agent: "designer", tokens: "boss reads too small; atmosphere is flat" },
    { delay: 1100, t: "critique", text: "Boss too small. Lighting is flat noon. No audio.", patch_lines: 5 },
...
export function startMock() {
  ...
  const runOne = async () => {
    ...
    for (const ev of seq) {
      ...
      const { delay: _delay, ...out } = ev;
      applyEvent({ ...out, sid } as Event);
    }
