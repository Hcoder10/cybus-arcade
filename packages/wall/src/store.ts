...
function ensureCurrent(sid: SessionId, init?: Partial<SessionView>): SessionView {
  ...
  const fresh: SessionView = {
    sid, from: init?.from ?? "", subject: init?.subject ?? "(building)",
    body: init?.body ?? "", startedAt: init?.startedAt ?? Date.now(),
...
export function applyEvent(e: Event) {
  ...
  if (e.t === "session.end") {
    ...
    cur.endedAt = Date.now(); cur.ok = e.ok; cur.shareUrl = e.share_url;
    completed.value = [...completed.value, cur];
    currentSession.value = null;
    return;
  }
  const sid = (e as any).sid as SessionId;
  if (!sid) return;
  const s = ensureCurrent(sid);
  if (e.t === "place.reset") {
    activate(s, "scheduler", "🧹 baseplate restored");
  } else if (e.t === "agent.thinking") {
    activate(s, e.agent, e.tokens);
  } else if (e.t === "agent.tool_call") {
    activate(s, e.agent, `→ ${e.tool}(${JSON.stringify(e.args).slice(0, 60)})`);
  } else if (e.t === "agent.result") {
    s.threads[e.agent] = { agent: e.agent, active: false, done: true, body: (e.ok ? "✓ " : "✗ ") + e.summary };
    currentSession.value = { ...s };
    ...
    currentSession.value = { ...s };
  } else if (e.t === "nia.indexed") {
    // ingestion is async post-session — surface a small toast on the most recent completed tile
    console.log("[nia.indexed]", e.title, "->", e.source_id);
  }
}
